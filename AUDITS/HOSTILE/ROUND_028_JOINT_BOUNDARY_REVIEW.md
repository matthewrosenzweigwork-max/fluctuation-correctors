...
terminal law is m_ij. For N>=3, I is a nonconstant function of Z, so
I and Z are not independent. At N=2 the unique mark is constant and
therefore independent of Z. Root identified the missing N>=3 qualification
in this review's unissued explanatory draft; the precise feedback and draft
are retained in the packet. Neither frozen claim nor constructor needed repair.
...
For nonzero k let F=N^-1 sum e_k, ell=c|k|^2, a=c+nu ell and
A_T=exp(-aT). Haar integration gives
...
frozen u_N,k:
    ...
    u_N,k=sqrt[(N-1)(1+2C_k log N)/(2N^3)]+2C_k(N-1)/N^2,

with order sqrt(1+log N)/N. This argument does not
...
Multiplication by N gives the exact frozen bound, with no lost factor:
