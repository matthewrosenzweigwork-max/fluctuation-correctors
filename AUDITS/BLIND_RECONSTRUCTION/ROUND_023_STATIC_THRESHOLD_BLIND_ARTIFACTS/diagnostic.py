...
def approximate_singular_probe():
    ...
    assert rows[-1]["absolute_error"] < 0.002
    assert all(row["H_N_over_m_squared"] < -2 for row in rows)
    assert abs(rows[-1]["wrong_without_background"]-target) > 1
    return {
