...
def main():
    ...
    proof='ROUND_021_CRITICAL_CUBIC_SYNTHESIS.md'
    assert (a/proof).read_bytes()==(a.parent/proof).read_bytes()
    result=json.loads((a/'EXACT_RESULTS.json').read_text())
    assert result['status']=='PASS' and result['assertions']==12531
    assert result['script_sha256']==sha((a/'round021_exact_checks.py').read_bytes())
