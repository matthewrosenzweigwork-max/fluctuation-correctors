...
def main():
    ...
    proof='ROUND_023_STATIC_THRESHOLD_INPUT_OBSTRUCTION.md'
    assert (a/proof).read_bytes()==(a.parent/proof).read_bytes()
    result=json.loads((a/'EXACT_RESULTS.json').read_text())
    assert result['status']=='PASS' and result['assertions']==1549
    assert result['code_sha256']==sha((a/'round023_static_checks.py').read_bytes())
