# Round 004: the coefficient-one diffusive comparison fails above s=d-2 — corrected submission

2026-09-17. Root self-check correction before external review: the original submission omitted one factor s in its displayed third derivative bound. This version changes both occurrences from s^3 to s^4; the original bytes and hash are preserved. The finite-time Taylor proof and theorem scope are unchanged. No promoted theorem or independent verdict is being revised.
...
where one possible completely explicit bound is
    ...
    C_F = [4 s^4 (2s+2)/N^2] r0^(-3s-4).

Indeed partial_tau F=s(r0^p+2sp tau/N)^(-s/p), its second derivative at zero is -(2s^3/N)r0^(-2s-2)=v.grad j, and its third derivative is [4s^4(2s+2)/N^2](r0^p+2sp tau/N)^(-s/p-2), whose maximum is attained at tau=0. This checks all time and N factors.
