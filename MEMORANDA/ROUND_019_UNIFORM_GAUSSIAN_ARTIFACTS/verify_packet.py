...
def main():
    ...
    proof='ROUND_019_UNIFORM_DIFFUSIVITY_GAUSSIAN.md'
    assert (a/proof).read_bytes()==(a.parent/proof).read_bytes()
    result=json.loads((a/'EXACT_RESULTS.json').read_text())
    assert result['status']=='PASS' and result['assertions']==22623
    assert result['script_sha256']==sha((a/'round019_exact_checks.py').read_bytes())
